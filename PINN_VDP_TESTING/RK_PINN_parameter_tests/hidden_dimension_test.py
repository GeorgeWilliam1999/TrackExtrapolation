...
##############################################################################
# 2) PINN Model (We will fix hidden_dim=16, but vary num_layers)
##############################################################################
...
# 6) Main Experiment: 
#    - Fix hidden_dim=16
#    - Vary number of layers (num_layers in [2,3,4,5,...])
#    - Train with early stopping
#    - Compute final-time error for multiple ICs
#    - Plot final-time error vs num_layers
##############################################################################
...
if __name__ == "__main__":
    ...
    # We fix hidden_dim=16, vary num_layers:
    layer_counts = [i for i in range(1,21)]
    ...
    # We'll store final-time errors in final_time_errors[nl] = [errIC0, errIC1, ...]
    final_time_errors = {}

    # 2) Loop over layer_counts
    hidden_dim = 16
    for nl in layer_counts:
        print(f"\n===== Training with num_layers={nl}, hidden_dim={hidden_dim} =====")
        model = RK_PINN(input_dim=2, hidden_dim=hidden_dim, output_dim=2, num_layers=nl)
        ...
        final_time_errors[nl] = errs

    # 3) Plot final-time error vs num_layers (lines for each IC)
    # Using Plotly
    fig = go.Figure()
    num_ics = len(ics)
    for ic_idx in range(num_ics):
        ...
        for nl in layer_counts:
            x_vals.append(nl)
            y_vals.append(final_time_errors[nl][ic_idx])
        fig.add_trace(
            go.Scatter(
                x=np.log(x_vals),
                y=np.log(y_vals),
    ...
    fig.update_layout(
        title="Final-Time Error vs. Number of Layers (hidden_dim=16, Early Stopping)",
    ...
    fig.write_html('PINN_VDP_TESTING/RK_PINN_parameter_tests/plots/hidden_dim_test.html')
    fig.show()
