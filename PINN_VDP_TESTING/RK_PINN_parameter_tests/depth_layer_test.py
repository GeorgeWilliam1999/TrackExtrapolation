...
##############################################################################
# 3) Build Training Data (y_n, y_{n+1})
##############################################################################
...
def compute_final_time_error(model, y0, t0, t_end, N_eval, mu=1.0, device="cpu"):
    ...
    print('RK: ', y_rk4[-1], 'NN: ', y_nn[-1])
    return torch.norm(y_rk4[-1] - y_nn[-1]).item()/torch.norm(y_rk4[-1]).item()
...
def train_model_until_convergence(
    ...
    """
    Train with a simple 'early stopping':
...
##############################################################################
# 6) Main: Vary Both (hidden_dim) and (num_layers), 3D Plot of Final-Time Error
##############################################################################
...
if __name__ == "__main__":
    ...
    t0, t_end = 0.0, 5.0
    mu = 1.0
    N_data = 100      # steps for building the training set
    N_eval = 100      # steps for measuring final-time error
    min_epochs = 200
    max_epochs = 2000000
    loss_tol = 1e-7
    patience = 100
    ...
    hidden_dims = [i for i in range(16, 65, 4)]
    ...
    # Build a single dataset from all ICs
    X_train_list = []
    Y_train_list = []
    for ic in ics:
    ...
    #store final-time errors in a 2D array: final_time_error[i, j]
    #   i -> index of hidden_dim
    #   j -> index of num_layers
    #compute the AVERAGE final-time error across all ICs for each (hd, nl).
    final_time_error_2d = []
    for i, hd in enumerate(hidden_dims):
        row_errors = []
        for j, nl in enumerate(layer_counts):
            print(f"\n=== Training model with hidden_dim={hd}, num_layers={nl} ===")
            model = RK_PINN(input_dim=2, hidden_dim=hd, output_dim=2, num_layers=nl)
            ...
            print(f"Finished at epoch {final_epoch}, final training loss={train_losses[-1]:.6e}")

            # Evaluate final-time error for each IC, then take average
            ic_errors = []
            for ic in ics:
                err_ft = compute_final_time_error(model, ic, t0, t_end, N_eval, mu=mu, device=device)
                ic_errors.append(err_ft)
                print(f"   IC={ic.tolist()} => final-time error={err_ft:.6e}")

            avg_err = sum(ic_errors) / len(ic_errors)
            print(f"   Average final-time error (over all ICs) = {avg_err:.6e}")

            row_errors.append(avg_err)
        final_time_error_2d.append(row_errors)


    # Convert to float arrays
    hidden_dims_np = np.log10(np.array(hidden_dims, dtype=float))
    layer_counts_np = np.log10(np.array(layer_counts, dtype=float))
    Z_np = np.log10(np.array(final_time_error_2d, dtype=float))
    Z_np = Z_np.T  # shape (len(layer_counts), len(hidden_dims))

    X2D, Y2D = np.meshgrid(hidden_dims_np, layer_counts_np)
    ...
    fig3d.update_layout(
        title="3D Surface of Final-Time Error (Averaged Over ICs)<br>"
              "X=Hidden Dim, Y=Num Layers, Z=Avg. Error",
        scene=dict(
            xaxis_title="Hidden Dimension",
            yaxis_title="Number of Layers",
            zaxis_title="Final-Time Error"
        ),
        width=800,
        height=700
    )
    fig3d.write_html('PINN_VDP_TESTING/RK_PINN_parameter_tests/plots/depth_layer_test.html')
    fig3d.show()
